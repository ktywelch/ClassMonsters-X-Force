...
let newMessageAlert;
let sendMessageBtn,  activeMess;
let newMessageCreateBtn,messList;
...
const handleMessView = (e) => {
  e.preventDefault();
  activeMess = JSON.parse(e.target.parentElement.getAttribute('data-'));
  renderActiveMess();
...
//////
const renderMessList = async (messsages) => {
  ...
  const createLi = (text, delBtn = true) => {
    ...
    spanEl.addEventListener('click', handleMesageView);
  ...
  if (window.location.pathname === '/teacher') {
    messListItems.forEach((message) => messList[0].append());
  }
};
...
const getAndRenderNotes = () => getNotes().then(renderMessList);
